...
@section('header')
    <div aria-label="breadcrumb">
        <ol class="breadcrumb  mb-1">
            ...
            <li class="breadcrumb-item active"><a href="{{ route('album.create') }}" class="text-decoration-none"><small>Add Album</small></a></li>
        </ol>
    </div>
            ...
            <form action="{{ route('album.store') }}" method="POST">
                ...
                <div class="col-sxl-7 col-xxl-8 col-xl-12 col-md-12 col-12 row">
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="artist" id="">
                            ...
                            @foreach ($artists as $item )
                                <option value="{{ $item->id }}" {{ old('artist', $artistId) == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                            @endforeach
                        </select>
                        @error('artist')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" placeholder="Enter Album Name" name="name" value="{{ old('name') }}">
                        @error('name')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="any"  class="form-control" placeholder="Enter Album Sales" name="sales" value="{{ old('sales') }}">
                        @error('sales')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" name="dateReleased" value="{{ old('dateReleased') }}">
                        @error('dateReleased')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="" class="form-label">Last Update<span class="text-danger">*</span></label>
                        <input type="date" class="form-control" name="lastUpdate" value="{{ old('lastUpdate') }}">
                        @error('lastUpdate')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
